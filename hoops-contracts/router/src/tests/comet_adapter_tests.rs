...
fn test_comet_adapter_swap_exact_in() {
    ...
    let amount_out = comet_adapter_client.swap_exact_in(
        &amount_in,
        &amount_out_min,
        &path,
        user,
...
#[test]
fn test_comet_adapter_swap_exact_out() {
    let test_env = HoopsTestEnvironment::setup();
    let env = &test_env.env;
    env.mock_all_auths();
    let user = &test_env.user;
    ...
    let initial_user_balance_a = token_a_client.balance(user);
    let initial_user_balance_b = token_b_client.balance(user);
    let amount_in_used = comet_adapter_client.swap_exact_out(
        &desired_out,
        &max_in,
        &path,
        user,
...
fn test_comet_adapter_add_and_remove_liquidity() {
    ...
    let lp_token = comet_adapter_client.add_liquidity(&token_a_client.address, &token_b_client.address, &amount_a, &amount_b, user, &deadline);
    let (amt_a_out, amt_b_out) = comet_adapter_client.remove_liquidity(&lp_token, &(amount_a.min(amount_b)), user, &deadline);
