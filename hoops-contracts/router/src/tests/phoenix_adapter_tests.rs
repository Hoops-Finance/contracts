...
pub mod phoenix_adapter_tests {
    ...
    fn test_phoenix_adapter_add_and_remove_liquidity() {
        ...
        let lp_token = phoenix_adapter_client.add_liquidity(&token_a_client.address, &token_b_client.address, &amount_a, &amount_b, user, &deadline);
        let (amt_a_out, amt_b_out) = phoenix_adapter_client.remove_liquidity(&lp_token, &(amount_a.min(amount_b)), user, &deadline);
