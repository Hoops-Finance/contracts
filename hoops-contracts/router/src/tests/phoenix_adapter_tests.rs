...
pub fn run_swap_exact_in(test_env: &HoopsTestEnvironment) {
    ...
    std::println!("[PHOENIX][swap_exact_in] Initial user balances: TKA = {}, TKB = {}", initial_user_balance_a, initial_user_balance_b);
    let amount_out = phoenix_adapter_client.swap_exact_in(&amount_in, &amount_out_min, &path, user, &deadline);
    std::println!("[PHOENIX][swap_exact_in] Swap result: amount_in = {}, amount_out = {}", amount_in, amount_out);
    let final_user_balance_a = token_a_client.balance(user);
    let final_user_balance_b = token_b_client.balance(user);
    std::println!("[PHOENIX][swap_exact_in] Final user balances: TKA = {}, TKB = {}", final_user_balance_a, final_user_balance_b);
...
pub fn run_swap_exact_out(test_env: &HoopsTestEnvironment) {
    ...
    let initial_user_balance_a = token_a_client.balance(user);
    let initial_user_balance_b = token_b_client.balance(user);
    std::println!("[PHOENIX][swap_exact_out] Initial user balances: TKA = {}, TKB = {}", initial_user_balance_a, initial_user_balance_b);
    let amount_in_used = phoenix_adapter_client.swap_exact_out(&desired_out, &max_in, &path, user, &deadline);
    std::println!("[PHOENIX][swap_exact_out] Swap result: max_in = {}, actual_in = {}, desired_out = {}", max_in, amount_in_used, desired_out);
    let final_user_balance_a = token_a_client.balance(user);
    let final_user_balance_b = token_b_client.balance(user);
    std::println!("[PHOENIX][swap_exact_out] Final user balances: TKA = {}, TKB = {}", final_user_balance_a, final_user_balance_b);
...
pub fn run_add_liquidity(test_env: &HoopsTestEnvironment) {
    ...
    let deadline = env.ledger().timestamp() + 100;
    let pool = test_env.phoenix.pool_ids.get(0).unwrap();
    ...
    std::println!("[PHOENIX][add_liquidity] User balances before: TKA = {}, TKB = {}", before_balance_a, before_balance_b);
    ...
    std::println!("[PHOENIX][add_liquidity] Result: amt_a = {}, amt_b = {}, lp = {}", amt_a, amt_b, lp);
    let after_balance_a = token_a_client.balance(user);
    let after_balance_b = token_b_client.balance(user);
    std::println!("[PHOENIX][add_liquidity] User balances after: TKA = {}, TKB = {}", after_balance_a, after_balance_b);
...
pub fn run_remove_liquidity(test_env: &HoopsTestEnvironment) {
    ...
    let amt_b_min: i128 = 1;
    let deadline = env.ledger().timestamp() + 100;
    std::println!("[PHOENIX][remove_liquidity] Removing liquidity: lp_amt = {}", lp_amt);
    let (amt_a_out, amt_b_out) = phoenix_adapter_client.remove_liquidity(&pool, &lp_amt, &amt_a_min, &amt_b_min, user, &deadline);
    std::println!("[PHOENIX][remove_liquidity] Result: amt_a_out = {}, amt_b_out = {}", amt_a_out, amt_b_out);
