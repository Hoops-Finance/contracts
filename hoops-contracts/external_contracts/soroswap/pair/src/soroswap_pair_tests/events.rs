...
use crate::soroswap_pair_tests::deposit::add_liquidity;
use crate::soroswap_pair_tests::SoroswapPairTest;
use soroban_sdk::{
    symbol_short,
...
fn withdraw_event() {
    ...
    let pair_token_client = SoroswapPairTokenClient::new(
        &test.env,
        &test.env.register_at(
            &test.contract.address,
            crate::SoroswapPairToken {},
            (),
        ),
    );
    ...
    // update to use register_at instead...
    test.env.register_at(&test.contract.address, crate::SoroswapPair {}, ());
