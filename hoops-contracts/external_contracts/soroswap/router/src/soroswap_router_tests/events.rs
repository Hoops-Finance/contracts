...
fn add_liquidity_event() {
    ...
    let (deposited_amount_0, deposited_amount_1, received_liquidity) =
        ...
        // not sure why but calling this function resets the events so let's try calling it natively.
    // let deterministic_pair_address = test.contract.router_pair_for(&test.token_0.address, &test.token_1.address);
    let deterministic_pair_address = soroswap_library::pair_for(test.env.clone(), test.factory.address.clone(), test.token_0.address.clone(), test.token_1.address.clone()).unwrap();

    std::println!(
        "[add_liquidity_event] All events before getting last: {:?}",
        test.env.events().all()
    );
