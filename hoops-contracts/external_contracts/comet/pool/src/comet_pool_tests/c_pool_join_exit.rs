...
use crate::{
    ...
    comet_pool_tests::{balancer::F64Utils, utils::assert_approx_eq_rel},
};

use super::{
    balancer::BalancerPool,
    utils::{create_comet_pool, create_token},
};

#[test]
fn test_comet_pool_join_exit() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let (token_3, token_3_admin) = create_token(&env, &admin);
    ...
    token_3_admin.mint(&admin, &balances.get_unchecked(2));
    let starting_bal: i128 = 100_000 * STROOP;
    token_1_admin.mint(&user, &starting_bal);
    token_2_admin.mint(&user, &starting_bal);
    token_3_admin.mint(&user, &starting_bal);

    let comet_id = create_comet_pool(
        &env,
        &admin,
        &vec![&env, token_1.address.clone(), token_2.address.clone(), token_3.address.clone()],
        &weights,
        &balances,
        0_0030000,
    );
    ...
    let post_join_bal_1 = token_1.balance(&user);
    let post_join_bal_2 = token_2.balance(&user);
    let post_join_bal_3 = token_3.balance(&user);
    let post_join_comet_bal_1 = token_1.balance(&comet_id);
    let post_join_comet_bal_2 = token_2.balance(&comet_id);
    let post_join_comet_bal_3 = token_3.balance(&comet_id);
    ...
    assert!(token_1.balance(&user) <= post_join_bal_1 + out_float_fixed.get_unchecked(0));
    assert!(token_2.balance(&user) <= post_join_bal_2 + out_float_fixed.get_unchecked(1));
    assert!(token_3.balance(&user) <= post_join_bal_3 + out_float_fixed.get_unchecked(2));
    assert!(
    ...
    assert!(
        token_3.balance(&comet_id)
            >= post_join_comet_bal_3 - out_float_fixed.get_unchecked(2)
    );
    assert_approx_eq_rel(
        token_1.balance(&user) - post_join_bal_1,
        out_float_fixed.get_unchecked(0),
        0_0001000,
    );
    assert_approx_eq_rel(
        token_2.balance(&user) - post_join_bal_2,
        out_float_fixed.get_unchecked(1),
        0_0001000,
    );
    assert_approx_eq_rel(
        token_3.balance(&user) - post_join_bal_3,
