...
use crate::{
    ...
    comet_pool_tests::{
        ...
        utils::{assert_approx_eq_abs, assert_approx_eq_rel, create_soroban_token, print_compare, create_token, create_comet_pool},
    },
};

#[test]
fn test_comet_pool_swap_out_given_in() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    // verify MAX_IN_RATIO
    let result =
        comet.try_swap_exact_amount_in(&token_1.address, &35_0000000, &token_2.address, &0, &i128::MAX, &user);
    assert_eq!(
        result.err(),
    ...
    let result = comet.try_swap_exact_amount_in(&token_1.address, &-1, &token_2.address, &0, &i128::MAX, &user);
    assert_eq!(
        result.err(),
        Some(Ok(Error::from_contract_error(
    ...
    // verify zero input
    let result = comet.try_swap_exact_amount_in(&token_1.address, &0, &token_2.address, &0, &i128::MAX, &user);
    assert_eq!(
        result.err(),
        Some(Ok(Error::from_contract_error(
            CometError::ErrNegativeOrZero as u32
    ...
    assert_eq!(token_2.balance(&user), starting_bal + res_2_out);
    assert_eq!(
...
#[test]
fn test_comet_pool_swap_in_given_out() {
    let env = Env::default();
    env.mock_all_auths();
    env.cost_estimate().budget().reset_unlimited();
    ...
    // verify negative input
    let result =
        comet.try_swap_exact_amount_out(&token_2.address, &i128::MAX, &token_1.address, &-2, &i128::MAX, &user);
    assert_eq!(
        result.err(),
        Some(Ok(Error::from_contract_error(
            CometError::ErrNegativeOrZero as u32
    ...
    // verify zero input
    let result =
        comet.try_swap_exact_amount_out(&token_2.address, &i128::MAX, &token_1.address, &0, &i128::MAX, &user);
    assert_eq!(
        result.err(),
        Some(Ok(Error::from_contract_error(
            CometError::ErrNegativeOrZero as u32
    ...
    let result = comet.try_swap_exact_amount_out(&token_2.address, &0, &token_1.address, &1, &i128::MAX, &user);
    assert_eq!(
        result.err(),
        Some(Ok(Error::from_contract_error(
            CometError::ErrNegativeOrZero as u32
    ...
    assert_eq!(token_2.balance(&user), starting_bal - res_2_in);
    assert_eq!(
        token_1.balance(&comet_id),
...
#[test]
fn test_comet_pool_swap_large_amounts() {
    ...
    let (res_out, _) =
        comet.swap_exact_amount_in(&token_2.address, &amount_fixed, &token_1.address, &0, &i128::MAX, &user);
...
#[test]
fn test_comet_pool_swap_large_price() {
    // test only validates recorded pool balances and assumes the above tests ensure that
    // ledger state is correct if the pool tracks internal balances correctly
    let env = Env::default();
    env.mock_all_auths();
    ...
    let (res_out, _) =
        comet.swap_exact_amount_in(&token_1.address, &amount_fixed, &token_2.address, &0, &i128::MAX, &user);
    assert!(res_out <= bal_out);
...
#[test]
fn test_comet_pool_swap_diff_decimals() {
    // test only validates recorded pool balances and assumes the above tests ensure that
    // ledger state is correct if the pool tracks internal balances correctly
    let env = Env::default();
    env.mock_all_auths();
